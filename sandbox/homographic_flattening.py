...
while True:
    ...
    if corners != ():
        ...
        if not (flat_corners != [0, 1] and flat_corners != [1, 0] and flat_corners != [0]):           
            ...
            tag_size = 20  # desired size of the ArUco tag in the output image
            ...
            homographies.append(adjusted_h_matrix)
            if len(homographies) > 2:
                homographies.pop(0)
            av_h = average_homographies(homographies)
            frame = cv2.warpPerspective(frame, av_h, (output_width, output_height)) # type: ignore
            ...
            if new_ids is not None and 0 in new_ids[0]:
                ...
                grid = ((frame / 255) * 100).astype(np.int8)
